...
parser::parser(std::vector<lexeme> &lexemes) : m_lexemes(lexemes) {
...
void parser::error_expected(const char *expected) {
  std::cerr << "Parser error in file " << next().infile_name << " line "
            << next().lineno << " at text " << next().lex << std::endl;
...
void parser::if_p(if_expression_t *sif) {
  ...
  if (next().token == ELSE) {
    ...
    statement_or_block(
        sif); // TODO: fix this, need a way to store a bunch of if else if ...
  }
...
binary_expression_t *parser::comp() {
  ...
  while (next().token == LT || next().token == LE || next().token == GT ||
         next().token == GE) {
    lexeme op = next();
    m_it++;
    right = sum();
...
int parser::type_check() {
  for (size_t i = 0; i < m_global_variables.size(); i++) {
    ...
    if (string_to_type(v->type) == VOID) {
      std::cerr << "Type checking error in file " << v->lex.infile_name
                << " line " << v->lex.lineno << " at text " << v->lex.lex
                << std::endl;
  ...
  for (size_t k = 0; k < m_functions.size(); k++) {
      ...
      for (size_t j = 0; j < f->params.size(); j++) {
        ...
        if (par->var == v->var) {
          std::cerr << "Type checking error in file " << v->lex.infile_name
                    << " line " << v->lex.lineno << " at text " << v->lex.lex
                    << std::endl;
          std::cerr << "\tvariable cannot have the same name as a parameter"
                    << std::endl;
    ...
    for (size_t i = 0; i < k; i++) {
      ...
      if (f->func == prev->func) {
        std::cerr << "Type checking error in file " << f->lex.infile_name
                  << " line " << f->lex.lineno << " at text " << f->func
                  << std::endl;
        std::cerr << "\tfunction with the same name already exists"
                  << std::endl;
...
parser_type parser::has_function(function_call_t *function) const {
  ...
  for (function_t *f : m_functions) {
    ...
    if (f->func == function->op.lex) {
      if (f->params.size() == function->args.size()) {
        ...
        for (size_t i = 0; i < f->params.size(); i++) {
          if (string_to_type(f->params[i]->type) !=
                  function->args[i]->derived_type ||
              f->params[i]->is_array != function->args[i]->is_array) {
            valid = false;
            break;
          }
        }
  ...
  if (function->op.lex == "putfloat") {
    if (function->args.size() == 1 &&
        function->args[0]->derived_type == FLOAT) {
      return FLOAT;
    }
  }

  if (function->op.lex == "putstring") {
    if (function->args.size() == 1 && function->args[0]->derived_type == CHAR &&
        function->args[0]->is_array) {
      return VOID;
    }
  }
